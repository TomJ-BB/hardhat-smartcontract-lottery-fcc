...
!developmentChains.includes(network.name) ? describe.skip : describe("Raffle Unit Tests", async function () {
    ...
    describe("performUpkeep", function() {
        ...
        it("reverts when upkeepNeeded is false", async function() {
            await raffle.enterRaffle({ value: raffleEntranceFee })
            await expect(raffle.performUpkeep([])).to.be.revertedWith(`Raffle__UpkeepNotNeeded(10000000000000000, 1, 0)`)
        })
...
//                        assert.equal(BigInt(winnerEndingBalance.toString()), BigInt(winnerStartingBalance + raffleEntranceFee * additionalEntrants + raffleEntranceFee).toString())
